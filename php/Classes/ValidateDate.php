 ...
 * It will convert a string representation to DateTime object or throw an exception.
 *
...
trait ValidateDate {
	 ...
	 *
	 * @param \DateTime|string $newDate date to valiadate
	...
	private static function validateDate($newDate) : \DateTime {
		...
		//treat the date as a mySQL date strings: Y-m-d
		$newDate = trim($newDate);
		if((preg_match("/^(\d{4})-(\d{2})$/", $newDate, $matches)) !== 1){
	 ...
	 * @throws \InvalidArgumentException if the date is in an invalid format
	 * @throws \RangeException if the date is not a Gregorian date
	 * @throws \TypeError when type hints fall
	...
	private static function validateTime(string $newTime) : string {
		...
		if ($hour < 0 || $hour >= 24 || $minute < 0 || $minute >= 60 || $second < 0 || $second >=60){
			throw(new \RangeException("date is not a valid wall"));
		}
